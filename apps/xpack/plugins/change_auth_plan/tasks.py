...
def test_change_auth_plan_task_connectivity(
    ...
    with tmp_to_root_org():
        ...
        for index, q in enumerate(queryset):
            print('{} Ход тестирования: ({}/{})'.format('>' * 10, index + 1, total))
            task_name = 'Проверьте возможность подключения запланированной задачи расшифровки: {}'.format(q)
            ...
            if dark:
                logger.info('Связность аутентификационной информации в тестовых задачах: неудачно')
                continue
            logger.info('Связность аутентификационной информации в тестовых задачах: успешно')
            if keep_auth_to_authbook:
                logger.info('Повторите попытку установить последнее состояние информации для проверки подлинности.')
