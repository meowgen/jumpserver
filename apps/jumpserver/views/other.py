...
class UIView(View):
    def get(self, request):
        msg = "Если вы видите эту страницу, это доказывает, что с вашей конфигурацией что-то не так, \
        пожалуйста, обратитесь к документации по настройке nginx, пользовательский интерфейс предоставлен проектом Lina."
        return HttpResponse(msg)
